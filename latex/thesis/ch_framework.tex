...
\begin{table}[!]
  ...
  \begin{tabular}{@{}llll@{}}
    ...
    man                & noun               & man              & num=sg            \\
    men                & noun               & man              & num=pl            \\
    woman              & noun               & woman            & num=sg            \\
    women              & noun               & woman            & num=pl            \\
    Jens               & proper noun        & Jens             &                   \\
    love               & transitive verb    & loves            & inf=inf           \\
    loves              & transitive verb    & loves            & inf=fin, num=sg   \\
    love               & transitive verb    & loves            & inf=fin, num=pl   \\
...
\paragraph{} \autoref{tbl:signaturen} vat alle signaturen nog eens samen. Elke inwendige knoop in de parse tree heeft nu al een betekenis in functie van de betekenis van haar directe kinderen. Er rest enkel nog de vraag wat de betekenis is van de woorden, de bladeren van de boom. Alle woorden moeten een een lambda-expressie als betekenis krijgen. Blackburn en Bos gebruiken hiervoor \textit{semantische macro's}. Dat wil zeggen dat ze voor elke lexicale categorie een functie hebben die een woordvorm uit het lexicon afbeeldt op een lambda-expressie. Hiervoor wordt de feature ``Symbool'' van het woord in kwestie gebruikt.
...
\begin{itemize}
  \item Een eigennaam kan vertaald worden als een constante met als naam het symbool dat bij die eigennaam hoort. $\sem{pn} = \lambdaf{P}{\app{P}{\textit{Symbool}}}$. Bijvoorbeeld voor ``Jens'': $\sem{Jens} = \lambdaf{P}{\app{P}{Jens}}$
  \item Een zelfstandig naamwoord test of een referentie voldoet aan het naamwoord zelf. $\sem{n} = \lambdaf{x}{\drs{}{\textit{Symbool}(x)}}$. Bijvoorbeeld voor ``man'': $\sem{man} = \lambdaf{x}{\drs{}{man(x)}}$
  \item Een lidwoord of meer algemeen een determinator introduceert een nieuwe referentie die een bepaalde scope heeft. Hier zijn er meerdere soorten vertalingen afhankelijk van het type van determinator. Een lidwoord heeft 2 argumenten: de \textit{restriction} en \textit{nuclear scope}. De \textit{restriction} wordt opgevuld wordt door het zelfstandig naamwoord (met eventuele bijzinnen). De \textit{nuclear scope} wordt opgevuld door de verbale constituent.
    \begin{itemize}
      \item De universele determinator (zoals ``every''): Het is duidelijk dat de variabele met een universele quantor gebonden moet zijn. In eerste-orde-logica krijgen we dan $\sem{det_{universeel}} = \lambdaf{R}{\lambdaf{S}{\forall x. \left( \app{R}{x} \Rightarrow \app{S}{x} \right)}}$. In DRS wordt dit $$\sem{det_{universeel}} = \lambdaf{R}{\lambdaf{S}{\drs{}{\drsImpl{\drsMerge{\drs{x}{}}{\app{R}{x}}}{\app{S}{x}}}}}$$
    ...
    \end{itemize}
  \item Een onovergankelijk werkwoord neemt de naamwoordgroep die het onderwerp vormt als argument. Het moet de referentie van die naamwoordgroep terug uit de box halen en testen of die referentie de actie van het werkwoord uitvoert $$\sem{iv} = \lambdaf{N}{\app{N}{\lambdaf{x}{\drs{}{\textit{Symbool}(x)}}}}$$
  \item Een overgankelijk werkwoord is gelijkaardig maar moet twee referenties uit hun box halen. Het eerste argument is het lijdend voorwerp, het tweede het onderwerp. $$\lambdaf{N1}{\lambdaf{N2}{\appB{N2}{\lambdaf{x2}{\appB{N1}{\lambdaf{x1}{\drs{}{\textit{Symbool}(x2, x1)}}}}}}}$$
\end{itemize}
