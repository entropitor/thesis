...
\todo{box aanmaken aanpassen naar iets om entiteit in vast te zetten??? WAT IS NU JUIST DE BOX???}
...
$$\sem{prep} = \lambdaf{N}{\lambdaf{P}{\left( \lambdaf{y}{\drsMerge{\app{P}{y}}{\app{N}{\lambdaf{x}{\drs{}{\textit{Symbool}(y, x)}}}}}} \right)}$$

Een voorzetsel neemt een naamwoordgroep $N$ en een substantief $P$ als argument en geeft een beperking op een entiteit $y$ terug. De beperking op $y$ bestaat enerzijds uit de beperking van het substantief ($\app{P}{y}$). Anderzijds voegt het voorzetsel zelf ook nog een beperking toe. Hiervoor haalt het de entiteit $x$ uit de box van de naamwoordgroep $N$ en linkt het de entiteiten $x$ en $y$ via een binair predicaat met dezelfde naam als het voorzetsel in kwestie. Het symbool van een voorzetsel is namelijk gelijk aan het voorzetsel zelf.

\section{Betrekkelijk voornaamwoord}
\todo{Gebruik S ipv P en B ipv R}
...
$$\sem{relpro} = \lambdaf{V}{\lambdaf{P}{\left( \lambdaf{x}{\drsMerge{\app{P}{x}}{\app{V}{\lambdaf{R}{\app{R}{x}}}}} \right)}}$$
Een betrekkelijk voornaamwoord neemt een verbale constituent $V$ en een substantief $P$ als argument en geeft een beperking op een entiteit $x$ terug. De beperkingen op $x$ bestaan enerzijds uit de beperking van het substantief $P$ (namelijk $\app{P}{x}$) en anderzijds uit de beperkingen van de verbale constituent. De verbale constituent heeft $x$ als onderwerp dus $V$ krijgt als argument een lege box rond $x$ mee ($\lambdaf{R}{\app{R}{x}}$)
